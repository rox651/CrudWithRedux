...
export const User = ({ user }) => {
  ...
  return (
   <tr className="border-b">
      <td className="text-sm text-gray-900 font-light px-6 py-4 whitespace-nowrap cursor-pointer" onClick={()=>dispatch(setCheck(id))}>{checkBox}</td>
      ...
      <td className="text-sm text-gray-900 font-light px-6 py-4 whitespace-nowrap flex gap-3">
        <button className="text-blue-500 font-black" onClick={()=>{
          dispatch(setUserEdit(user))
          dispatch(openForm())
        }}>Editar</button>
        <button onClick={()=>{dispatch(removeData(id))}} className="text-red-500 font-black">Eliminar</button>
      </td>
    </tr>
  );
};
